...
{
    WNDCLASSEX wc = {
        sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(NULL), NULL, NULL, NULL, NULL, L"RebootClass",
        NULL
    };
    ::RegisterClassEx(&wc);
    HWND hwnd = ::CreateWindowExW(0L, wc.lpszClassName, L"Project Reboot",
                                  (WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_MAXIMIZEBOX), 100, 100,
    ...
    while (!done)
    {
        ...
        if (!ImGui::IsWindowCollapsed())
        {
            ...
            auto InitializePlayers = [&Players]()
            {
                // TODO: slow
            ...
            if (ImGui::BeginTabBar(""))
            {
                ...
                if (Server::Status == Server::StatusType::Up)
                {
                ...
                if (FortniteVersion >= 8.51 || FortniteVersion == 4.1)
                {
            ...
            if (PlayerTab == -1)
            {
                ...
                case 1:
                    {
                        ImGui::Checkbox(("Log RPCS"), &LogRpcs);
                        ImGui::Checkbox(("Log ProcessEvent"), &LogProcessEvent);

                        // ImGui::Checkbox(("Use Beacons"), &bUseBeacons);

                        if (Server::Status == Server::StatusType::Down && !NetHooks::Traveled)
                        {
                            // TODO: Map name
                        ...
                        if (EngineVersion == 422 && ImGui::Button("Summon Floorloot"))
                        {
                            CreateThread(0, 0, Looting::SummonFloorLoot, 0, 0, 0);
                        }
                        ...
                        if (Server::Status == Server::StatusType::Up)
                        {
                                ...
                                if (ImGui::Button(("Start Aircraft")))
                                {
                                    ...
                                    if (Helper::IsSmallZoneEnabled())
                                    {
                                        ...
                                        if (Aircraft)
                                        {
                                            ...
                                            if (FlightInfo)
                                            {
                                                static auto FlightSpeedOffset = FindOffsetStruct(
                                                    "ScriptStruct /Script/FortniteGame.AircraftFlightInfo",
                                                    "FlightSpeed");
                                                static auto FlightStartLocationOffset = FindOffsetStruct(
                                                    "ScriptStruct /Script/FortniteGame.AircraftFlightInfo",
                                                    "FlightStartLocation");

                                                auto FlightSpeed = (float*)(__int64(FlightInfo) + FlightSpeedOffset);
                                                auto FlightStartLocation = (FVector*)(__int64(FlightInfo) +
                                                    FlightStartLocationOffset);
                                                ...
                                                if (true) // RandomPOI)
                                                {
                                                    GameMode::AircraftLocation = Helper::GetActorLocation(
                                                        RandomFoundation) + FVector{0, 0, 5000};

                                                    *FlightStartLocation = GameMode::AircraftLocation;
                                                    Helper::SetActorLocation(Aircraft, GameMode::AircraftLocation);
                                                    std::cout << std::format(
                                                        "Set aircraft location to {} {} {}\n",
                                                        GameMode::AircraftLocation.X, GameMode::AircraftLocation.Y,
                                                        GameMode::AircraftLocation.Z);
                                                }
                                                else
                                                    std::cout << "No POI!\n";
                            ...
                            {
                                CreateThread(0, 0, Looting::SummonLamas, 0, 0, 0);
                            }

                            if (ImGui::Button("Fill Vending Machines"))
                            {
                                CreateThread(0, 0, Looting::FillVendingMachines, 0, 0, 0);
                            }
                        }
                        ...
                        if (ImGui::Button("Dump Playlists")) // iirc ftext changed but idk
                        {
                            ...
                            {
                                PlaylistsFile << "Fortnite Version: " + std::to_string(FortniteVersion) << "\n\n";
                                ...
                                for (int32_t i = 0; i < (ObjObjects ? ObjObjects->Num() : OldObjects->Num()); i++)
                                {
                                    ...
                                    if (Object && Object->IsA(FortPlaylistClass))
                                    {
                                        ...
                                        std::string PlaylistName = Helper::Conversion::TextToString(
                                            *Object->Member<FText>("UIDisplayName"));
                        ...
                        if (ImGui::Button("Dump Weapons")) // iirc ftext changed but idk
                        {
                            ...
                            if (WeaponsFile.is_open())
                                WeaponsFile << "Fortnite Version: " + std::to_string(FortniteVersion) << "\n\n";
                            static auto FortWeaponItemDefinitionClass = FindObjectOld(
                                "Class /Script/FortniteGame.FortWeaponItemDefinition", true);
                            ...
                            for (int32_t i = 0; i < (ObjObjects ? ObjObjects->Num() : OldObjects->Num()); i++)
                            {
                                ...
                                {
                                    // std::string PlaylistName = Object->Member<FName>("PlaylistName")->ToString(); // Short name basically
                                    std::string ItemDefinitionName = Helper::Conversion::TextToString(
                                        *Object->Member<FText>("DisplayName"));
                ...
                case 2:
                    ...
                    for (int i = 0; i < Players.size(); i++)
                    {
                        ...
                        if (ImGui::Button(wstring_to_utf8(std::wstring(
                                Helper::GetfPlayerName((*Player.first->Member<UObject*>("Controller"))).Data.GetData()))
                            .
                            c_str()))
                        {
                            PlayerTab = i;
                ...
                case 3:
                    {
                        if (!Started)
                            ImGui::InputText(("Playlist"), &GameMode::PlaylistName);
                ...
                case 4:
                    if (Started == true)
                    {
                        if (FortniteVersion >= 8.51 && ImGui::Button(("Init New Thanos")))
                        {
                            Ltm::Thanos::InitHooks(false);
                        }
                        if (FortniteVersion >= 8.51 && ImGui::Button(("Spawn Stone")))
                        {
                            Ltm::Thanos::SpawnRandomStone();
                        }

                        if (FortniteVersion == 4.1 && ImGui::Button(("Init Thanos")))
                        {
                            Ltm::Thanos::InitHooks(true);
                        }
                        if (FortniteVersion == 4.1 && ImGui::Button(("Spawn Gauntlet")))
                        {
                            Ltm::Thanos::SpawnGauntlet();
                        }
                    }
                    else
                    {
                        ImGui::Text("Press The Button under this text to setup Thanos for your Version!");
                        if (FortniteVersion == 4.1 && ImGui::Button(("Init Carmine")))
                        {
                            GameMode::PlaylistName =
                                "FortPlaylistAthena /Game/Athena/Playlists/Carmine/Playlist_Carmine.Playlist_Carmine";
                        }
                        if (FortniteVersion >= 8.51 && ImGui::Button(("Init Ashton")))
                        {
                            GameMode::PlaylistName =
                                "FortPlaylistAthena /Game/Athena/Playlists/Ashton/Playlist_Ashton_Lg.Playlist_Ashton_Lg";
                        }
                    }
                ...
                case 5:
                    ...
                    if (FortniteVersion == 8.51)
                    {
                        ImGui::InputText("Item to Unvault (DrumGun, Bouncer, Sword, Grappler, Tac)",
                                         &Event::Butterfly::ItemToUnvault);

                        if (ImGui::Button("Unvault Item"))
                        {
                            FString InStr;
                            InStr.Set(std::wstring(Event::Butterfly::ItemToUnvault.begin(),
                                                   Event::Butterfly::ItemToUnvault.end()).c_str());
                            Event::Butterfly::Finish(Helper::Conversion::StringToName(InStr));
                        }
                    }

                    if (FortniteVersion == 12.41 && ImGui::Button("Fly players up"))
                        Event::Travis::Start();

                    if (FortniteVersion == 6.21)
                    {
                ...
                case 6: // settings
                    ...
                    ImGui::InputText("First Slot", &Item::StartingSlot1.first);
                    ImGui::InputInt("First Slot Amount", &Item::StartingSlot1.second);
                    ImGui::NewLine();
                    ImGui::InputText("Second Slot", &Item::StartingSlot2.first);
                    ImGui::InputInt("Second Slot Amount", &Item::StartingSlot2.second);
                    ImGui::NewLine();
                    ImGui::InputText("Third Slot", &Item::StartingSlot3.first);
                    ImGui::InputInt("Third Slot Amount", &Item::StartingSlot3.second);
                    ImGui::NewLine();
                    ImGui::InputText("Fourth Slot", &Item::StartingSlot4.first);
                    ImGui::InputInt("Fourth Slot Amount", &Item::StartingSlot4.second);
                    ImGui::NewLine();
                    ImGui::InputText("Fifth Slot", &Item::StartingSlot5.first);
                    ImGui::InputInt("Fifth Slot Amount", &Item::StartingSlot5.second);
                    ImGui::NewLine();
                ...
                if (PlayerTab < Players.size())
                {
                    ...
                    if (CurrentPlayer.first && CurrentPlayer.second)
                    {
                        ...
                        if (!bInformationTab)
                        {
                            ...
                            ImGui::TextColored(ImVec4(18, 253, 112, 0.8),
                                               (("Player: ") + PlayerName.ToString()).c_str());
                        ...
                        else
                        {
                            ...
                            TextCentered(std::format(("X: {} Y: {} Z: {}"), (int)PawnLocation.X, (int)PawnLocation.Y,
                                                     (int)PawnLocation.Z));
                            // We cast to an int because it changes too fast. 
                            ...
                            if (CurrentWeapon)
                            {
                                ...
                                if (Guid)
                                {
                                    ...
                                    if (Controller)
                                    {
                                        auto ItemInstances = Player::GetItems(Controller);
                                        ...
                                        if (ItemInstances)
                                        {
                                            for (int j = 0; j < ItemInstances->Num(); j++)
                                            {
                                                ...
                                                auto CurrentGuid = Item::GetGuid(ItemInstance);

                                                if (CurrentGuid == *Guid)
                                                {
                                                    Definition = Item::GetDefinition(ItemInstance);
                                                    Count = *Item::GetCount(
                                                        ItemInstance->Member<__int64>(("ItemEntry")));
                                                    // lets hope its a UFortWorldItem*
                                                    // break;
                                                }
                                            }
                                            ...
                                            if (Definition)
                                            {
                                                ...
                                                if (Name)
                                                {
                                                    ...
                                                    TextCentered(std::format(
                                                        "Holding {} {}", bMultiple ? std::format("{}x", Count) : "a",
                                                        bMultiple ? DisplayName + "s" : DisplayName));
                                                    // TODO: Add Count and Rarity
                                                }
                                            }
                                        }
                                    }
