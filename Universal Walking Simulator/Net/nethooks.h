	...
	*(char*)(__int64(PlayerState) + bIsReadyToContinueOffset) = true;
...

	{
		...
		if (!Helper::HasAircraftStarted() || !bClearInventoryOnAircraftJump)
		{
			...
			Inventory::GiveGrapplerAmmo(PlayerController);
			if (bIsFloorIsLava == true)
			{
				Inventory::GiveLavaMats(PlayerController);
			}

			if (bIs999Ammo == true)
			{
				Inventory::Give999Ammo(PlayerController);
			}

			if (bIs999Mats == true)
			{
				Inventory::Give999Mats(PlayerController);
			}
		}
	}
